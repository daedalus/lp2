...
from lp2.transpiler.lean4_to_python import lean_to_py as lean_to_py_ast
from lp2.transpiler.python_to_lean4 import py_to_lean as py_to_lean_ast
...
class TestLeanCodegenCommands:
    ...
    def test_inductive_with_fields(self):
        m = LeanModule(imports=[], body=[
            LeanInductive(
                name="Foo",
                params=[],
                type=LeanSort(None),
                constructors=[
                    LeanConstructor(name="mk", fields=[LeanParam(name="x", type=LeanIdent("Int"))]),
    ...
    def test_instance_with_methods(self):
        m = LeanModule(imports=[], body=[
            LeanInstance(
                ...
                methods=[LeanDef(name="x", params=[], return_type=LeanIdent("Int"), value=LeanNum(42))],
            )
        ])
        out = generate_lean(m)
    ...
    def test_axiom(self):
        m = LeanModule(imports=[], body=[
            LeanAxiom(name="foo", params=[LeanParam(name="x", type=LeanIdent("Int"))], type=LeanIdent("Bool"))
        ])
        out = generate_lean(m)
        assert "axiom foo" in out

    def test_example(self):
        m = LeanModule(imports=[], body=[LeanExample(expr=LeanBinOp(LeanNum(1), "+", LeanNum(2)))])
        out = generate_lean(m)
        assert "#eval" in out

    def test_variable(self):
        m = LeanModule(imports=[], body=[
            LeanVariable(params=[LeanParam(name="x", type=LeanIdent("Int"))])
        ])
        out = generate_lean(m)
    ...
    def test_namespace(self):
        m = LeanModule(imports=[], body=[
            LeanNamespace(name="Foo", commands=[LeanDef(name="bar", params=[], return_type=LeanIdent("Int"), value=LeanNum(1))])
        ])
        out = generate_lean(m)
    ...
    def test_section_with_params(self):
        m = LeanModule(imports=[], body=[
            LeanSection(params=[LeanParam(name="x", type=LeanIdent("Int"))], commands=[])
        ])
        out = generate_lean(m)
        assert "section" in out

    def test_theorem_and_lemma(self):
        thm = LeanDef(name="thm1", params=[], return_type=LeanIdent("Nat"), value=LeanNum(0), is_theorem=True)
        lem = LeanDef(name="lem1", params=[], return_type=LeanIdent("Nat"), value=LeanNum(1), is_lemma=True)
    ...
    def test_sig_only_def(self):
        d = LeanDef(name="f", params=[LeanParam(name="x", type=LeanIdent("Int"))], return_type=LeanIdent("Bool"), value=None)
...
class TestLeanCodegenExprs:
    def test_char(self):
        m = LeanModule(imports=[], body=[LeanDef(name="c", params=[], return_type=None, value=LeanChar('a'))])
        out = generate_lean(m)
        assert "'a'" in out

    def test_sort_with_level(self):
        m = LeanModule(imports=[], body=[LeanDef(name="t", params=[], return_type=None, value=LeanSort(level=LeanNum(1)))])
        out = generate_lean(m)
        assert "Type" in out

    def test_unary_op_alpha(self):
        m = LeanModule(imports=[], body=[LeanDef(name="f", params=[], return_type=None, value=LeanUnaryOp(op="not", operand=LeanBool(True)))])
        out = generate_lean(m)
        assert "not true" in out

    def test_type_arrow(self):
        m = LeanModule(imports=[], body=[LeanDef(name="f", params=[], return_type=None, value=LeanTypeArrow(from_type=LeanIdent("Int"), to_type=LeanIdent("Bool")))])
        out = generate_lean(m)
        assert "→" in out

    def test_type_spec(self):
        m = LeanModule(imports=[], body=[LeanDef(name="f", params=[], return_type=None, value=LeanTypeSpec(expr=LeanNum(5), type=LeanIdent("Int")))])
        out = generate_lean(m)
        assert ":" in out or "Int" in out

    def test_forall_with_and_without_type(self):
        with_type = LeanForall(params=[LeanParam(name="x", type=LeanIdent("Int"))], body=LeanIdent("True"))
        without_type = LeanForall(params=[LeanParam(name="x", type=None)], body=LeanIdent("True"))
        m = LeanModule(imports=[], body=[
    ...
    def test_pi(self):
        m = LeanModule(imports=[], body=[LeanDef(name="f", params=[], return_type=None, value=LeanPi(binder=LeanParam(name="x", type=LeanIdent("Int")), body=LeanIdent("Bool")))])
        out = generate_lean(m)
        assert "→" in out

    def test_list_lit(self):
        m = LeanModule(imports=[], body=[LeanDef(name="xs", params=[], return_type=None, value=LeanListLit(elts=[LeanNum(1), LeanNum(2)]))])
        out = generate_lean(m)
        assert "[1, 2]" in out

    def test_tuple_lit(self):
        m = LeanModule(imports=[], body=[LeanDef(name="t", params=[], return_type=None, value=LeanTupleLit(elts=[LeanNum(1), LeanIdent("x")]))])
        out = generate_lean(m)
        assert "(1, x)" in out
    ...
    def test_proj(self):
        m = LeanModule(imports=[], body=[LeanDef(name="f", params=[], return_type=None, value=LeanProj(expr=LeanIdent("p"), field="x"))])
        out = generate_lean(m)
        assert "p.x" in out

    def test_have_with_name(self):
        have = LeanHave(name="h", type=LeanIdent("Nat"), value=LeanNum(1), body=LeanIdent("h"))
        m = LeanModule(imports=[], body=[LeanDef(name="f", params=[], return_type=None, value=have)])
        out = generate_lean(m)
        assert "have h" in out

    def test_show(self):
        show = LeanShow(type=LeanIdent("Nat"), value=LeanNum(1))
        m = LeanModule(imports=[], body=[LeanDef(name="f", params=[], return_type=None, value=show)])
        out = generate_lean(m)
    ...
    def test_calc(self):
        ...
        m = LeanModule(imports=[], body=[LeanDef(name="f", params=[], return_type=None, value=calc)])
        out = generate_lean(m)
        assert "calc" in out
    ...
    def test_do_all_branches(self):
        ...
        bind = LeanDo(stmts=[LeanDoBind(pattern=LeanPatternIdent("x"), value=LeanApp(LeanIdent("f"), LeanNum(1)))], last=LeanIdent("x"))
        do_expr = LeanDo(stmts=[LeanDoExpr(expr=LeanApp(LeanIdent("f"), LeanNum(1)))], last=None)
        m = LeanModule(imports=[], body=[
    ...
    def test_by(self):
        ...
        m = LeanModule(imports=[], body=[LeanDef(name="f", params=[], return_type=None, value=by_expr)])
        out = generate_lean(m)
        assert "by simp" in out

    def test_parenthesized(self):
        m = LeanModule(imports=[], body=[LeanDef(name="f", params=[], return_type=None, value=LeanParenthesized(expr=LeanNum(1)))])
        out = generate_lean(m)
        assert "(1)" in out

    def test_named_arg(self):
        m = LeanModule(imports=[], body=[LeanDef(name="f", params=[], return_type=None, value=LeanNamedArg(name="x", value=LeanNum(1)))])
        out = generate_lean(m)
        assert "x := 1" in out

    def test_gen_expr_none(self):
        m = LeanModule(imports=[], body=[LeanDef(name="f", params=[], return_type=None, value=None)])
        out = generate_lean(m)
        assert "def f" in out
    ...
    def test_pattern_ctor_with_subpatterns(self):
        pat = LeanPatternCtor(name="Foo", patterns=[LeanPatternIdent("a"), LeanPatternNum(1)])
        from lp2.codegen.lean4_codegen import _gen_pattern
...
class TestPyCodegenStatements:
    def test_ann_assign_with_and_without_value(self):
        with_val = PyAnnAssign(target=PyName("x"), annotation=PyName("int"), value=PyConstant(1))
        no_val = PyAnnAssign(target=PyName("y"), annotation=PyName("int"), value=None)
        m = PyModule(body=[PyFunctionDef("f", [("z", None, None)], None, [with_val, no_val])])
        code = generate_python(m)
        assert "x: int = 1" in code
        assert "y: int" in code

    def test_if_elif_else_chain(self):
        inner_if = PyIf(test=PyCompare(PyName("x"), ["=="], [PyConstant(2)]), body=[PyPass()], orelse=[])
        outer_if = PyIf(test=PyCompare(PyName("x"), ["=="], [PyConstant(1)]), body=[PyPass()], orelse=[inner_if])
        m = PyModule(body=[PyFunctionDef("f", [("x", PyName("int"), None)], None, [outer_if])])
        code = generate_python(m)
        assert "elif" in code
        assert "if" in code
    ...
    def test_while_loop(self):
        m = PyModule(body=[PyFunctionDef("f", [], None, [
            PyWhile(test=PyConstant(True), body=[PyBreak()])
        ])])
        code = generate_python(m)
    ...
    def test_continue(self):
        m = PyModule(body=[PyFunctionDef("f", [], None, [
            PyFor(target=PyName("i"), iter=PyName("items"), body=[PyContinue()])
        ])])
        code = generate_python(m)
    ...
    def test_try_except_else_finally(self):
        m = PyModule(body=[PyFunctionDef("f", [], None, [
            PyTry(
                body=[PyPass()],
                handlers=[PyExceptHandler(typ=PyName("Exception"), name="e", body=[PyPass()])],
                orelse=[PyPass()],
                finalbody=[PyPass()],
            )
        ])])
    ...
    def test_assert_with_and_without_msg(self):
        with_msg = PyAssert(test=PyCompare(PyName("x"), ["=="], [PyConstant(1)]), msg=PyConstant("bad"))
    ...
    def test_delete(self):
        m = PyModule(body=[PyFunctionDef("f", [], None, [
            PyDelete(targets=[PyName("x"), PyName("y")])
        ])])
        code = generate_python(m)
    ...
    def test_nested_if_elif_else_chain(self):
        ...
        code = generate_python(PyModule(body=[PyFunctionDef("f", [("x", PyName("int"), None)], None, [elif_chain])]))
        assert "elif" in code


class TestPyCodegenExprs:
    def test_yield_and_yield_from(self):
        m = PyModule(body=[PyFunctionDef("gen", [("n", PyName("int"), None)], PyName("int"), [
    ...
    def test_await(self):
        m = PyModule(body=[PyFunctionDef("f", [], None, [
            PyReturn(value=PyAwait(value=PyCall(func=PyName("get"), args=[PyName("url")]))),
        ])])
        code = generate_python(m)
    ...
    def test_starred(self):
        m = PyModule(body=[PyFunctionDef("f", [], None, [
            PyReturn(value=PyCall(func=PyName("foo"), args=[PyStarred(value=PyName("items"))])),
        ])])
        code = generate_python(m)
    ...
    def test_dict_comp(self):
        comp = PyDictComp(key=PyName("k"), value=PyName("v"), generators=[
    ...
    def test_walrus(self):
        m = PyModule(body=[PyFunctionDef("f", [], None, [
            PyExprStmt(expr=PyWalrus(target=PyName("x"), value=PyConstant(1))),
        ])])
        code = generate_python(m)
        assert "x := 1" in code
    ...
    def test_unknown_expr_raises(self):
        from lp2.codegen.python_codegen import _gen_expr, _gen_stmt
    ...
    def test_gen_pattern_py_fallbacks(self):
        ...
        assert _gen_pattern_py(PyMatchOr(patterns=[PyConstant(1), PyConstant(2)])) == "1 | 2"
        assert _gen_pattern_py(PyCall(func=PyName("int"), args=[PyName("x")])) == "int(x)"
...
""")
    ...
    def test_match_bool_ctor_patterns(self):
        result = lean_to_py("def f (b : Bool) : Int := match b with\n  | true => 1\n  | false => 0\n")
        assert result
...
""")
    ...
    def test_empty_arms_match(self):
        result = lean_to_py("def f : Int := match true with\n  | true => 1\n  | false => 0\n")
        assert result
    ...
    def test_structure_no_annot_field(self):
        ...
        s = LeanStructure(name="Point", params=[], extends=[], fields=[LeanParam(name="x", type=None)])
    ...
    def test_prod_mk_two_elements(self):
        from lp2.transpiler.lean4_to_python import _expr_to_py
        app = LeanApp(func=LeanApp(func=LeanIdent("Prod.mk"), arg=LeanNum(1)), arg=LeanNum(2))
