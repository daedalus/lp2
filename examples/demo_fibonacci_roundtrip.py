#!/usr/bin/env python3
"""
Demo: Round-trip transpilation of Fibonacci (Python → Lean → Python)
...
def main():
    print("=== LP2: Python ↔ Lean Fibonacci Demo ===\n")

    with open(os.path.join(os.path.dirname(__file__), "fibonacci.py")) as f:
    ...
    eval_code = "\n".join(f"#eval fibonacci {n}" for n in [0, 1, 10, 20])
    result = subprocess.run(
        ["lean", "--stdin"],
        input=lean_source + "\n" + eval_code,
        capture_output=True,
    ...
    if result.returncode == 0:
        ...
        for n, out in zip([0, 1, 10, 20], outputs):
            print(f"  fibonacci({n}) = {out}")
