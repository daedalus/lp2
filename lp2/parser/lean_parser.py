...
PRECEDENCE = {
    ...
    'COLON': 2,
    'PIPEOP': 3,
    'EQ': 4, 'DEQ': 4, 'LT': 4, 'GT': 4, 'LE': 4, 'GE': 4, 'NE': 4,
    'AMP': 5,
    'PIPE': 6,
    'DOUBLECOLON': 7,
    'PLUS': 8, 'MINUS': 8,
...
BINARY_OPS = {
    ...
    'EQ': '=', 'DEQ': '==', 'LT': '<', 'GT': '>', 'LE': '≤', 'GE': '≥', 'NE': '≠',
    'DOUBLECOLON': '::', 'PLUSPLUS': '++',
    'AMP': '&&', 'PIPE': '||', 'HAT': '^', 'RARROW': '→',
    'PIPEOP': '|>',
}
...
class LeanParser:
    ...
    def _expect(self, kind: str, value: Optional[str] = None) -> Token:
        ...
        if t.kind != kind or (value is not None and t.value != value):
            raise SyntaxError(f"Expected {kind}({value}), got {t.kind}({t.value}) at line {t.line}, col {t.col}")
        ...
        while self._peek().kind != 'EOF':
            if self._peek().kind == 'OPEN':
                ...
                try:
                    ...
                    self._skip_to({'EOF', 'DEF', 'THEOREM', 'LEMMA', 'INDUCTIVE',
    ...
    def _parse_def(self) -> LeanDef:
        ...
        elif self._peek().kind in ('DEF', 'THEOREM', 'LEMMA', 'ID', 'EOF',
        ...
        return LeanDef(
            ...
            value=value, is_theorem=is_theorem, is_lemma=is_lemma,
            is_mutual=is_mutual, sig_only=sig_only
        )
    ...
    def _parse_binders(self) -> list[LeanParam]:
        ...
        while self._peek().kind in ('LPAREN', 'LBRACE', 'LBRACK'):
            ...
            close_kind = {'LPAREN': 'RPAREN', 'LBRACE': 'RBRACE', 'LBRACK': 'RBRACK'}[open_kind]
            self._expect(close_kind)

            for name in names:
                params.append(LeanParam(
                    name=name, type=typ, explicit=explicit
                ))
    ...
    def _parse_inductive(self) -> LeanInductive:
        ...
        while self._peek().kind == 'ID':
            ...
            self._maybe('PIPE')
        return LeanInductive(name=name, params=params, type=typ, constructors=constructors)
    ...
    def _parse_instance(self) -> LeanInstance:
        ...
        if self._peek().kind in ('COLONEQ', 'EQ'):
            self._advance()
        methods = []
        while self._peek().kind == 'DEF' or (self._peek().kind == 'ID' and self._peek().value != 'end'):
    ...
    def _parse_namespace(self) -> LeanNamespace:
        ...
        while self._peek().kind not in ('EOF',) and self._peek().value != 'end':
            commands.append(self._parse_def() if self._peek().kind == 'DEF' else
                           self._parse_inductive() if self._peek().kind == 'INDUCTIVE' else
    ...
    def _parse_expr(self, min_prec: int = 0) -> LeanExpr:
        ...
        if lhs is None:
            t = self._peek()
            raise SyntaxError(f"Unexpected token {t.kind}({t.value}) at line {t.line}, col {t.col}")
        ...
        while True:
            ...
            elif tok.kind == 'ID' or tok.kind == 'NUM' or tok.kind == 'STR' or \
                 tok.kind == 'BOOL' or tok.kind == 'FLOAT' or tok.kind == 'CHAR' or \
                 tok.kind == 'LPAREN' or tok.kind == 'LBRACK' or tok.kind == 'LBRACE' or \
                 tok.kind == 'WILD':
    ...
    def _parse_prefix(self) -> Optional[LeanExpr]:
        ...
        if tok.kind == 'ID':
            ...
            return LeanNum(value=val, is_nat=val >= 0)
        elif tok.kind == 'FLOAT':
            val = float(self._advance().value)
            return LeanFloat(value=val)
        elif tok.kind == 'STR':
    ...
    def _parse_lambda(self) -> LeanLambda:
        ...
        while self._peek().kind in ('ID', 'LPAREN', 'LBRACE', 'WILD'):
            if self._peek().kind == 'ID':
                ...
                params.extend(self._parse_binders())
        self._expect('ARROW') if self._peek().kind == 'ARROW' else self._expect('RARROW')
    ...
    def _parse_forall(self) -> LeanForall:
        self._advance()
        params = []
        while not (self._peek().kind in ('COMMA', 'ARROW', 'RARROW', 'DOT') or
                   (self._peek().kind == 'ID' and self.tokens[self.pos+1].kind == 'COLON')):
    ...
    def _parse_match(self) -> LeanMatch:
        ...
        if self._peek().kind == 'PIPE':
            self._maybe('PIPE')
        arms = []
        while self._peek().kind in ('ID', 'NUM', 'WILD', 'LPAREN', 'BOOL', 'STR', 'CHAR') or \
              (self._peek().kind == 'PIPE' and self.tokens[self.pos+1].kind in ('ID', 'NUM', 'WILD', 'LPAREN')):
            if self._peek().kind == 'PIPE':
                self._advance()
    ...
    def _parse_let(self) -> LeanLet:
        ...
        self._expect('IN')
        body = self._parse_expr()
        return LeanLet(name=name, params=params, type=typ, value=value, body=body, is_mut=is_mut)
    ...
    def _parse_calc(self) -> LeanCalc:
        self._expect('CALC')
        steps = []
        while self._peek().kind != 'EOF' and self._peek().kind not in ('DEF', 'THEOREM', 'INDUCTIVE', 'STRUCTURE'):
    ...
    def _parse_do(self) -> LeanDo:
        ...
        stmts = []
        last = None
        while self._peek().kind not in ('EOF', 'DEF', 'THEOREM', 'INDUCTIVE', 'STRUCTURE',
                                          'CLASS', 'INSTANCE', 'AXIOM'):
            ...
            elif self._peek().kind == 'ID' and self.tokens[self.pos+1].kind in ('ARROW', 'COLONEQ'):
                pat = self._parse_pattern()
                arrow = self._advance()
                val = self._parse_expr()
                stmts.append(LeanDoBind(pattern=pat, value=val))
            elif self._peek().kind == 'WILD' and self.tokens[self.pos+1].kind in ('ARROW', 'COLONEQ'):
                pat = self._parse_pattern()
                self._advance()
                val = self._parse_expr()
                stmts.append(LeanDoBind(pattern=pat, value=val))
