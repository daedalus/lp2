...
KW_MAP = {
    'def': 'DEF', 'theorem': 'THEOREM', 'lemma': 'LEMMA',
    'inductive': 'INDUCTIVE', 'structure': 'STRUCTURE', 'class': 'CLASS',
    'instance': 'INSTANCE', 'axiom': 'AXIOM', 'example': 'EXAMPLE',
    'open': 'OPEN', 'namespace': 'NAMESPACE', 'section': 'SECTION',
    'variable': 'VARIABLE', 'mutual': 'MUTUAL', 'partial': 'PARTIAL',
    'nonrec': 'NONREC', 'private': 'PRIVATE', 'protected': 'PROTECTED',
    'public': 'PUBLIC',
    'fun': 'FUN', 'λ': 'FUN', 'fun ': 'FUN',
    'forall': 'FORALL', '∀': 'FORALL',
    'match': 'MATCH', 'with': 'WITH', '=>': 'ARROW',
    'if': 'IF', 'then': 'THEN', 'else': 'ELSE',
    'let': 'LET', 'in': 'IN',
    'have': 'HAVE', 'show': 'SHOW', 'calc': 'CALC',
    'do': 'DO',
    'return': 'RETURN',
    'mutual': 'MUTUAL', 'mut': 'MUT',
    'where': 'WHERE',
    ...
    '{': 'LBRACE', '}': 'RBRACE',
    '_': 'WILD',
    '#eval': 'HASH_EVAL', '#check': 'HASH_CHECK',
    '#print': 'HASH_PRINT',
}
...
class LeanLexer:
    ...
    def _tokenize(self):
        while self.pos < len(self.source):
            ...
            multi = {'->': 'RARROW', '=>': 'ARROW', '::': 'DOUBLECOLON',
            ...
            unicode_single = {'→': 'RARROW', '≤': 'LE', '≥': 'GE', '≠': 'NE',
                              '∀': 'FORALL', 'λ': 'FUN'}
            ...
            single = {
                ...
                '.': 'DOT', '|': 'PIPE', '=': 'EQ', '+': 'PLUS', '-': 'MINUS',
                '*': 'STAR', '/': 'SLASH', '%': 'PERCENT', '^': 'HAT',
                '<': 'LT', '>': 'GT', '!': 'BANG', '@': 'AT', '$': 'DOLLAR',
                '~': 'TILDE', '&': 'AMP', '?': 'QMARK', '#': 'HASH',
            }
            if ch in single:
                self._advance()
