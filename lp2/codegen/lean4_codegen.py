...
def _gen_expr(node: LeanExpr) -> str:
    ...
    if isinstance(node, LeanIdent):
        ...
        def needs_parens(n: LeanExpr) -> bool:
            return isinstance(n, (LeanApp, LeanBinOp, LeanUnaryOp,
                                   LeanLambda, LeanIf, LeanMatch, LeanLet,
                                   LeanTypeArrow, LeanTypeSpec))
        ...
        params = ' '.join(f'({p.name} : {_gen_expr(p.type)})' if p.type else p.name for p in node.params)
        ...
        return f'{{ {node.struct_name} with {fields} }}' if node.fields else f'{{ {node.struct_name} }}'
    elif isinstance(node, LeanProj):
        return f'{_gen_expr(node.expr)}.{node.field}'
    elif isinstance(node, LeanMatch):
        arms = '\n    '.join(f'| {_gen_pattern(arm.pattern)} => {_gen_expr(arm.rhs)}' for arm in node.arms)
        ...
        steps = '\n    '.join(f'{_gen_expr(s.relation)} := {_gen_expr(s.value)}' for s in node.steps)
        return f'calc\n    {steps}'
